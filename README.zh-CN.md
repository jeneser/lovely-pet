...
当前版本开箱即用：克隆仓库后可以直接运行一个布偶猫示例。该示例使用 `Resources/pets/default/frames` 下由 `pet.json` 驱动的透明 PNG 图片帧；运行时不再用 SwiftUI 程序化图形绘制宠物形象。轻量互动包括 hover、点击、双击、长按、触摸分区、亲密度记忆、缩放设置和睡眠节奏。
...
- 鼠标追踪、触摸分区、亲密度记忆、睡眠状态、缩放记忆和窗口位置记忆
- manifest 配置和校验脚本
...
当前重点是在 PNG 图片帧播放基础上提升桌宠的自然感和陪伴感：更自然的 hover、鼠标感知移动、触摸分区反馈、轻量亲密度记忆、睡眠节奏、可预测的本地状态、Xcode 友好调试和稳定的 macOS 打包体验。
