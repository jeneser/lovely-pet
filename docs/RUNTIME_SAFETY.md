...
- Shows a transparent floating desktop window.
- Adds a menu bar item with Settings and Quit controls.
- Loads transparent PNG frames from bundled resources; no external renderer process is started.
...
- PNG frame playback uses per-state FPS timers from `pet.json`.
- macOS Reduce Motion is respected for lightweight transform effects around the image frames.
