...
This asset set uses transparent PNG sprite sheets generated from the supplied ragdoll-cat references. Every subframe is normalized to the app window canvas (`320x340`), and `pet.json` addresses each runtime frame with `sheet.png#index` references.

## Pose groups

| Runtime group | Sheet frames | Role |
|---|---:|---|
| `frames/idle/sheet.png` | `#0`–`#14` | Neutral breathing, blink, reopen, tail sway, and return-to-stand in-betweens |
| `frames/hover/sheet.png` | `#0`–`#7` | Notice, ears-perk, head tilt, forward lean, paw probe, paw hold, and recovery |
| `frames/tap/sheet.png` | `#0`–`#9` | Startle, hop/landing, paw-settle, attention recovery, and return-to-idle transition |
| `frames/sleep/sheet.png` | `#0`–`#6` | Lowering, paw tuck, loaf, breathing, and sleep twitch loop |
